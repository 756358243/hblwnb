...
public class PdaController {
	...
	*/
	...
	public ResponseEntity<String> wjxxEnd(HttpServletRequest req) throws Exception {
		...
		String jsonData = StrUtil.getParamStr(req, "jsonData", "");
		//1.调用service的方法
	    JsonResult result = pdaService.wjxxAllEnd(jsonData);
	...
	*/
	...
	public ResponseEntity<String> underpanStart(HttpServletRequest req) throws Exception {
		...
		//检测站编号
		String CUNITCODE = StrUtil.getParamStr(req, "CUNITCODE", ""); 
		
		//1.0调用service的方法
		JsonResult result =pdaService.insertUnderpanStart(CCHECKCODE, CPREFACECHECKID, CCHKDEVICECODE, CUNITCODE);
	...
	*/
	...
	public ResponseEntity<String> underpanEnd(HttpServletRequest req) throws Exception {
		...
		//根据流水号出入信息
		String CCHECKCODE = StrUtil.getParamStr(req, "CCHECKCODE", "");
	    //调用service层的时候，用通道类别作为区分是哪个视频，流水号是一样的
		
		JsonResult result =pdaService.insertUnderpanEnd(CCHECKCODE);
